...
export default function Profile() {
  ...
  useEffect(() => {
    ...
    if (session?.user?.email) {
      ...
      getCourses(session.user.email).then(courses => {
      ...
      getTasks(session.user.email).then(tasks => {
        ...
        <form onSubmit={handleSave} className={`bg-[var(--bg-light)] rounded-2xl shadow-xl border border-[var(--border)] p-4 sm:p-6 md:p-8 flex flex-col gap-4 sm:gap-6 md:gap-8 transition-all duration-700 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <div className="flex flex-col items-center gap-4 sm:gap-6">
            <div className="relative group">
              <img
                src={photo || "/profile-placeholder.png"}
                alt="Profile"
                className="w-20 h-20 sm:w-24 sm:h-24 md:w-32 md:h-32 rounded-full object-cover border-4 border-[var(--primary)] bg-[var(--bg)] shadow-lg group-hover:shadow-2xl group-hover:scale-105 transition-all duration-300 cursor-pointer"
              />
              <button 
                type="button" 
